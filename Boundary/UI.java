...
public abstract class UI {
	...
	//Scanner for getting inputs from user
	...
	public String getInputString() {
		...
		//Condition for obtaining a valid input
		boolean handled = false;
		//Continue to prompt the user if input entered is not a String and String does not only contain letters
		do {
			try {
	...
	public String getInputString(String[] stringToCheck) {
		...
		//Condition for obtaining a valid input
		boolean handled = false;
		//Continue to prompt the user if input entered is not a String and the String entered does not exist in the string array
		do {
			try {
				userInput = sc.nextLine();
	...
	//If check == false, it checks if the ID exist. If it exist, it is not usable as it is trying to obtain a new non-negative ID: CHECK FOR NON-EXISTING ID
	//If check == true, it checks if the ID exist. If it exist, the id is valid for use: CHECKING FOR EXISTING ID
	public int getValidID(boolean check) 
	{
		...
		//Condition for obtaining a valid input
		boolean handled = false;
		//Continue to prompt the user if input entered is not an Integer and it is not a valid ID
		do {
			try {
				userInput = sc.nextInt();
				sc.nextLine();
	...
	//Obtain input as a valid phone number from 8xxx xxxx - 9xxx xxxx (Valid SG Phone Number)
	//If check == true, it checks if the contact number exist. If it exist, it is not usable as it is trying to obtain a new valid contact number: CHECK FOR NON-EXISTING ID
	//If check == false, it checks if the contact number exist. If it exist, the contact number is valid for use: CHECKING FOR EXISTING ID
	public int getValidContactNumber(boolean check) 
	{
		...
		//Condition for obtaining a valid input
		boolean handled = false;
		//Continue to prompt the user if input entered is not an Integer and it is not a valid Phone number
		do {
			try {
				userInput = sc.nextInt();
				sc.nextLine();
				...
				{
					//Check if the number already exist in the database (NO DUPLICATE PHONE NUMBERS)
					...
					//Input entered is an existing phone number in the database
						...
						System.out.println("Error! The phone number already exist for another person, please ensure you typed in the correct phone number");
					else
						System.out.println("Error! The phone number does not belong to any existing person in the database");
				}
				//Input enter is not a number between 8xxx xxxx - 9xxx xxxx
				else
				{
					System.out.println("Error! Please enter a valid Phone Number (8xxxxxxx - 9xxxxxxx) ! Please try again.");
				}
	...
	//Combining all the options available as YES & NO for checking if input is valid
